    ...
    <div className="flex min-h-screen w-screen items-center justify-center bg-slate-950 px-4 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md glass-panel p-8 border-slate-800/80 bg-slate-900/40 relative z-10 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          {error && (
            <div className="bg-rose-500/10 border border-rose-500/20 text-rose-455 p-4 rounded-xl flex items-center gap-3 animate-in fade-in">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full astro-button-primary py-3.5 mt-2"
          >
            {loading ? (
        ...
        <div className="mt-8 text-center text-sm text-slate-450 border-t border-slate-800/80 pt-6">
          Already registered?{' '}
          <Link to="/login" className="text-astrology-400 hover:text-astrology-300 font-semibold underline underline-offset-4">
            Sign In Instead
          </Link>
        </div>
      </div>
